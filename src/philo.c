...
void	philo_eat(t_philo *philo)
{
	...
	if (philo->data->finished == 0)
		printf("%lld %d has taken a fork.\n", cur_time(&philo->start), philo->philo_id);
...
void	philo_check(t_data *data)
{
	...
	while (1)
	{
		...
		if (data->philo_str[i].eat_n == data->max_eat)
			break ;
		if (cur_time(&data->start) - data->philo_str[i].last_time_eaten > data->time_to_die)
		{
			printf("%lld %d has died\n", cur_time(&data->start), data->philo_str[i].philo_id);
			data->finished = 1;
